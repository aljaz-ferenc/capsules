					...
					{[...links, { title: "Reserve", scrollToId: "welcome" }].map(
						(link) => (
							<Link
								...
								key={link.title}
								globalScroll={false}
								className="link [clip-path:inset(0%_0%_100%_0%)] main-text-large text-muted md:hover:text-white hover:transition"
							/>
						),
					)}
				</div>

				<div className="flex flex-col justify-end h-full opacity-0" id="bottom">
					<p className="main-text-small py-[30px] max-w-[30ch] px-[20px]">
						Meet Capsules®—modern and cozy houses, in the California desert.
					</p>
					<SocialLinks className="mb-10 px-[20px]" />
					<div className="max-h-[300px] h-full rounded-[30px] overflow-hidden relative">
						<img
							src="terrace.webp"
							alt=""
